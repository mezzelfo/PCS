    	...
    	for(unsigned int cellId = 0; cellId < meshPointer->NumberOfCells(); cellId++)
        {
            if (cellsToCut.at(cellId) == true)
            {
                ...
                const GenericPoint* Pp = cell->Point(2); // Punto opposto al lato lungo
                 ...
                 GenericEdge* L1 = (GenericEdge*)cell->Edge(1);  //Lato (1) del triangolo
                 GenericEdge* L2 = (GenericEdge*)cell->Edge(2);  //Lato (2) del triangolo
                ...
                // Celle che insistono sui lati L1 e L2 all'interno
                L1->InsertCell(C1,1);
                L2->InsertCell(C0,1);
                ...
                C0->AllocateCells(3);
                C0->InsertCell(C1,0);
                C0->InsertCell(L2->RightCell(),1);
                //C0->InsertCell(bho0,2);           TODO

                // Seconda sottocella
                C1->AllocateCells(3);
                //C1->InsertCell(bho0,0);           TODO
                C1->InsertCell(L1->RightCell(),1);
                C1->InsertCell(C0,2);
