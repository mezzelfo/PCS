...
namespace GeDiM
{
	Output::ExitCodes TriangleRefiner::RefineMesh()
	{
		...
		{
			edgeId = (edgeId+1)%meshPointer->NumberOfEdges();    // Il modulo è necessario per poter scorrere il vettore in modo circolare
			if (not idEdgesToCut.at(edgeId))                     // Se il lato non è da tagliare lo ignoro e passo oltre
				continue;

			GenericEdge* L = meshPointer->Edge(edgeId);          // Creo un puntatore al lato da tagliare L

			if (IsOnBorder(L))                                   // Se è un lato di bordo della mesh
			{
				GenericCell* C0;
				...
				idEdgesToCut.at(edgeId) = false;                 // Indico a idEdgesToCut che il lato non è più da tagliare
			}
			else if (LongestEdge(L->Cell(0)) == LongestEdge(L->Cell(1)))    // Se il lato è il più lungo di entrambi i triangoli a cui appartiene
			{
	...
	{
		GenericEdge* L = meshPointer->Edge(LongestEdge(C)->Id());      // Creo un puntatore al lato più lungo del triangolo
		...
		// Ruoto i punti fino a quando quello in posizione 2 non appartiene più al lato più lungo L
	...
	void TriangleRefiner::PrepareTriangle(const unsigned int& value)
	{
		...
		const GenericEdge* L = LongestEdge(C);               // Creo un puntatore al lato più lungo L
	...
	void TriangleRefiner::RefinePairedTriangles(GenericCell* C0, GenericCell* C1)
	{
		...
		RotateCell(C0);
		RotateCell(C1);
		// Creo il puntatore al lato più lungo di entrambe
		...
		GenericEdge* subEdge0 = NewEdge();	    // Figli del lato più lungo
	...
	void TriangleRefiner::RefineBorderTriangle(GenericCell* C0)
	{
		...
		GenericEdge* longest = meshPointer->Edge(LongestEdge(C0)->Id());  // Creo il puntatore al lato più lungo
		...
		C0_0->AddCell(C0_1);
		PensaciTuAlLatoIgnoto(C0_0, meshPointer->Edge(C0->Edge(2)->Id()));
		C0_0->AddCell(NULL);    // Il triangolo è di bordo
		...
		PensaciTuAlLatoIgnoto(C0_1, meshPointer->Edge(C0->Edge(1)->Id()));
		C0_1->AddCell(C0_0);
		C0_1->AddCell(NULL);    // Il triangolo è di bordo
